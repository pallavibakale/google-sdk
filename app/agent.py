...
# 1. Create a basic agent that can answer stock-related questions
basic_agent = LlmAgent(
    model=LiteLlm(model="openai/gpt-3.5-turbo"),
    name="basic_agent",
    description="Stock market assistant",
    instruction="Answer stock-related questions. If unsure, say 'I don't know'."
)

# 2. Create a tool agent that can fetch stock prices using yfinance
...
tool_agent = LlmAgent(
    name='tool_agent',
    model=LiteLlm(model="openai/gpt-3.5-turbo"),
    description='A tool agent that can fetch stock prices',
    instruction="""You are a stock market price assistant. Use the get_stock_price tool to fetch stock prices. Include the ticker symbol in your response.""",
    tools=[get_stock_price]
)

# 3. Add state management to the tool agent to remember recent searches
from google.adk.agents import Agent
